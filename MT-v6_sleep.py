    ...
    while(connectionThreadCounter != 3):
        if serial_conThread is not None:
            if serialCon.is_connected() and conCheck.serial is False:
                ...
                serialTransmit_Thread = RPIThread(function = serialTransmit, name='serialTransmit-Thread')
                serialTransmit_Thread.daemon = True
                serialTransmit_Thread.start()
                print("serial transmit started")
